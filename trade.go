...
func (t *Trade) HnapayNewSelfAccount(request *proto.HnapayNewSelfAccountReq) (*proto.HnapayNewSelfAccountResp, error) {
	...
	client := proto.NewTradeClient(conn)
	fmt.Printf("client: %+v, conn: %+v \n", client, conn)

	ctx := GetMetadataCtx(t.RequestId, t.BizCode, t.Token)
	resp, err := client.HnapayNewSelfAccount(ctx, request)
...
func (t *Trade) SetPaymentSecret(request *proto.SetPaymentSecretReq) (*proto.SetPaymentSecretResp, error) {
	conn, err := NewGrpcClient(t.Url)
	if err != nil {
		return nil, err
	}
